...
namespace FlightReadinessEngine.Api.Agents
{
    public class MaintenanceAgent
    {
        ...
        private const string CachePrefix = "maintenance_agent";
        private const string ModelId = "gemini-2.5-flash";

        public MaintenanceAgent(ILogger<MaintenanceAgent> logger, IAgentCache cache)
        {
            _logger = logger;
            _cache = cache;
        ...
        {
            _logger.LogInformation("--- [MAINTENANCE SUBSYSTEM ENGINE] Scanning BigQuery for Table Updates ---");
            ...
            if (!updatedTails.Any())
            {
                _logger.LogInformation("[MAINTENANCE SUBSYSTEM] No new row changes or cache mismatches detected.");
                return new { message = "Maintenance data is already synchronized. No analysis needed." };
            }

            _logger.LogInformation($"[MAINTENANCE SUBSYSTEM] Processing updates for {updatedTails.Count} aircraft tail(s): {string.Join(", ", updatedTails)}");
            var maintenanceAssessments = new List<object>();

            foreach (var tail in updatedTails)
            {
                _logger.LogInformation($"[MAINTENANCE SUBSYSTEM] Deploying Maintenance Agent for Tail: {tail}...");
                string structuredAgentOutput = await ExecuteMaintenanceAgentWithTools(tail);
                var parsedJson = JsonSerializer.Deserialize<JsonElement>(structuredAgentOutput);
                maintenanceAssessments.Add(isTargetedRequest ? BuildTargetedResponse(tail, parsedJson) : parsedJson);
            }
            ...
            return new
            {
                ...
                updatedMaintenanceAssessments = maintenanceAssessments
            };
        }
        ...
        private async Task<object> RunTargetedCacheFirstAsync(List<string> targetTails)
        {
            ...
            foreach (var tail in targetTails)
            {
                if (!latestMarkers.TryGetValue(tail, out var currentMarker) || string.IsNullOrWhiteSpace(currentMarker))
                {
                    assessments.Add(new
                    {
                        ...
                        domainRiskSummary = "No maintenance row found for requested tail.",
                ...
                string rowJson = await TalkToMaintenanceTable(tail);
                if (!string.IsNullOrWhiteSpace(rowJson) && rowJson != "{}")
                {
                    await SafeSetAsync(rowKey, rowJson);
                }

                string assessmentJson = await EvaluateMaintenanceFromRowAsync(tail, rowJson);
            ...
            {
                systemStatus = "SYNCHRONIZATION_COMPLETE",
                updatedMaintenanceAssessments = assessments
            };
        }
        ...
        private async Task<List<string>> ScanTableForChanges(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.maintenance_analytics` GROUP BY tail";
                parameters = Enumerable.Empty<BigQueryParameter>();
            }
            else if (targetFlightIds.Count == 1)
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.maintenance_analytics` WHERE tail = @flightId GROUP BY tail";
                parameters = new[] { new BigQueryParameter("flightId", BigQueryDbType.String, targetFlightIds[0]) };
            }
            else
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.maintenance_analytics` WHERE tail IN UNNEST(@flightIds) GROUP BY tail";
            ...
            foreach (var row in results)
            {
                ...
                string cacheKey = $"maintenance_agent:flight:{flightId}:last_seen";
                string legacyCacheKey = $"maintenance_agent:flightId:{flightId}:last_seen";
                ...
                if (!cacheUnavailable)
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(cachedTimestampStr)) cachedTimestampStr = await _cache.GetStringAsync(legacyCacheKey);
                    }
                    catch (RpcException ex) when (ex.StatusCode == Grpc.Core.StatusCode.PermissionDenied)
                    {
                        cacheUnavailable = true;
                        _logger.LogError("[MAINTENANCE SUBSYSTEM] Cache access denied. Continuing without cache. Verify GCP IAM for Datastore service account.");
                    }
                }
                ...
                if (!cacheUnavailable)
                {
                    ...
                    {
                        cacheUnavailable = true;
                        _logger.LogError("[MAINTENANCE SUBSYSTEM] Cache write denied. Continuing without cache. Verify GCP IAM for Datastore service account.");
                    }
                }
            }
        ...
        private async Task<string> SafeGetAsync(string key)
        {
            ...
            {
                _logger.LogWarning(ex, "[MAINTENANCE SUBSYSTEM] Cache read failed for key {CacheKey}", key);
                return string.Empty;
            }
        }
        ...
        private async Task SafeSetAsync(string key, string value)
        {
            ...
            {
                _logger.LogWarning(ex, "[MAINTENANCE SUBSYSTEM] Cache write failed for key {CacheKey}", key);
            }
        }
        ...
        private async Task<string> EvaluateMaintenanceFromRowAsync(string tail, string rowJson)
        {
                ...
                Contents =
                {
                    new Content
                    {
                        ...
                        Parts =
                        {
                            ...
                            {
                                Text =
                                    $"You are the Maintenance Operations Agent. Analyze the row for tail {tail}. " +
            ...
            GenerateContentResponse response = await Services.VertexRetry.InvokeAsync(
                () => client.GenerateContentAsync(request), _logger, "Maintenance");

            return response.Candidates[0].Content.Parts[0].Text;
        }

        private async Task<string> ExecuteMaintenanceAgentWithTools(string tail)
        {
            var client = await new PredictionServiceClientBuilder
            {
                Endpoint = $"{_location}-aiplatform.googleapis.com",
            ...
            toolParametersSchema.Properties.Add("flightId", new OpenApiSchema { Type = Google.Cloud.AIPlatform.V1.Type.String, Description = "The target aircraft tail identifier (e.g. N12345)" });

            var queryTool = new Tool
            {
                FunctionDeclarations = { new FunctionDeclaration { Name = "QueryMaintenanceTable", Description = "Queries maintenance_analytics for tail-level maintenance readiness and work-order status.", Parameters = toolParametersSchema } }
            };
            ...
            {
                Model = $"projects/{_projectId}/locations/{_location}/publishers/google/models/{ModelId}",
                Contents = { new Content { Role = "USER", Parts = { new Part { Text = $"You are the Maintenance Operations Agent. An update has occurred. Execute your table query tool for tail {tail}, review maintenance_status, work_order_status, maintenance_issue and last_updated_at, and provide your safety risk verification assessment." } } } },
                Tools = { queryTool },
                GenerationConfig = new GenerationConfig { ResponseMimeType = "application/json", ResponseSchema = responseSchema }
            };
            ...
            {
                _logger.LogInformation($" -> [TOOL USE] Maintenance Agent requested execution of tool: {responsePart.FunctionCall.Name}");
                string requestedTail = responsePart.FunctionCall.Args.Fields["flightId"].StringValue;

                string tableDataJson = await TalkToMaintenanceTable(requestedTail);
                ...
                var toolResponseRequest = new GenerateContentRequest
                {
                    ...
                    Contents =
                    {
                        ...
                        {
                            Role = "USER",
                            Parts = { new Part { FunctionResponse = new FunctionResponse { Name = "QueryMaintenanceTable", Response = structuredResponseData } } }
                        }
                    },
                ...
                GenerateContentResponse finalAgentDecision = await Services.VertexRetry.InvokeAsync(
                    () => client.GenerateContentAsync(toolResponseRequest), _logger, "Maintenance");
        ...
        private async Task<string> TalkToMaintenanceTable(string tail)
        {
            BigQueryClient client = await BigQueryClient.CreateAsync(_projectId, Services.GcpAuth.GetCredential());
            string query = @"SELECT tail, last_updated_at, maintenance_status, work_order_status, maintenance_issue FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.maintenance_analytics` WHERE tail = @flightId ORDER BY last_updated_at DESC LIMIT 1";
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                maintenance_status = row["maintenance_status"]?.ToString(),
                work_order_status = row["work_order_status"]?.ToString(),
                maintenance_issue = row["maintenance_issue"]?.ToString()
            });
        }
