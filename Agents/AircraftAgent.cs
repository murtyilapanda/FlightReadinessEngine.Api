...
namespace FlightReadinessEngine.Api.Agents
{
    public class AircraftAgent
    {
        ...
        private async Task<List<string>> ScanTableForChanges(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.aircraft_analytics` GROUP BY tail";
                parameters = Enumerable.Empty<BigQueryParameter>();
            }
            else if (targetFlightIds.Count == 1)
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.aircraft_analytics` WHERE tail = @flightId GROUP BY tail";
                parameters = new[] { new BigQueryParameter("flightId", BigQueryDbType.String, targetFlightIds[0]) };
            }
            else
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.aircraft_analytics` WHERE tail IN UNNEST(@flightIds) GROUP BY tail";
        ...
        {
            BigQueryClient client = await BigQueryClient.CreateAsync(_projectId);
            string query = @"SELECT tail, last_updated_at, fleet_type, current_airport, scheduled_departure, fuel_ticket_status, fuel_ticket_number, apu_is_running, apu_runtime_mins, apu_within_guidelines FROM `zinc-hour-460015-n7.aviation_ops_analytics.aircraft_analytics` WHERE tail = @flightId ORDER BY last_updated_at DESC LIMIT 1";
