...
namespace FlightReadinessEngine.Api.Agents
{
    public class FlightPlanningAgent
    {
        ...
        private async Task<List<string>> ScanTableForChanges(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.flight_planning_analytics` WHERE tail = @flightId GROUP BY tail";
                parameters = new[] { new BigQueryParameter("flightId", BigQueryDbType.String, targetFlightIds[0]) };
            }
            else
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.flight_planning_analytics` WHERE tail IN UNNEST(@flightIds) GROUP BY tail";
        ...
        {
            BigQueryClient client = await BigQueryClient.CreateAsync(_projectId);
            string query = @"SELECT tail, last_updated_at, flight_plan_status, flight_number, flight_plan_issue FROM `zinc-hour-460015-n7.aviation_ops_analytics.flight_planning_analytics` WHERE tail = @flightId ORDER BY last_updated_at DESC LIMIT 1";
