...
namespace FlightReadinessEngine.Api.Agents
{
    public class PartsAgent
    {
        ...
        public async Task<object> RunAsync(AgentSyncRequest? request)
        {
            ...
            if (!updatedTails.Any())
            {
                _logger.LogInformation("[PARTS SUBSYSTEM] No new row changes detected.");
                return new { message = "Parts data is already synchronized. No analysis needed." };
            }

            _logger.LogInformation($"[PARTS SUBSYSTEM] Found updates for {updatedTails.Count} aircraft tail(s): {string.Join(", ", updatedTails)}");
        ...
        private async Task<List<string>> ScanTableForChanges(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.parts_analytics` GROUP BY tail";
                parameters = Enumerable.Empty<BigQueryParameter>();
            }
            else if (targetFlightIds.Count == 1)
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.parts_analytics` WHERE tail = @flightId GROUP BY tail";
                parameters = new[] { new BigQueryParameter("flightId", BigQueryDbType.String, targetFlightIds[0]) };
            }
            else
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.parts_analytics` WHERE tail IN UNNEST(@flightIds) GROUP BY tail";
        ...
        {
            BigQueryClient client = await BigQueryClient.CreateAsync(_projectId);
            string query = @"SELECT tail, last_updated_at, parts_status, part_required, part_available, part_in_transit, parts_issue FROM `zinc-hour-460015-n7.aviation_ops_analytics.parts_analytics` WHERE tail = @flightId ORDER BY last_updated_at DESC LIMIT 1";
        ...
        private async Task UpdateCacheMarkersForTails(List<string> tails)
        {
            try
            {
                ...
                {
                    string query = @"SELECT CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `zinc-hour-460015-n7.aviation_ops_analytics.parts_analytics` WHERE tail = @flightId";
                    var parameters = new[] { new BigQueryParameter("flightId", BigQueryDbType.String, tail) };
                    BigQueryResults rows = await client.ExecuteQueryAsync(query, parameters);
                    ...
                    {
                        await _cache.SetStringAsync($"parts_agent:flight:{tail}:last_seen", marker);
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning($"[PARTS SUBSYSTEM] Could not update cache markers after targeted request: {ex.Message}");
            }
        }
    }
}
