...
namespace FlightReadinessEngine.Api.Agents
{
    public class PartsAgent
    {
        ...
        {
            _logger.LogInformation("--- [PARTS SUBSYSTEM ENGINE] Scanning BigQuery for Table Updates ---");
            ...
            if (!updatedTails.Any())
            {
                _logger.LogInformation("[PARTS SUBSYSTEM] No new row changes or cache mismatches detected.");
                return new { message = "Parts data is already synchronized. No analysis needed." };
            }

            _logger.LogInformation($"[PARTS SUBSYSTEM] Processing updates for {updatedTails.Count} aircraft tail(s): {string.Join(", ", updatedTails)}");
            var partsAssessments = new List<object>();
            ...
            return new
            {
                ...
                updatedPartsAssessments = partsAssessments
            };
        }
        ...
        private async Task<object> RunTargetedCacheFirstAsync(List<string> targetTails)
        {
            ...
            foreach (var tail in targetTails)
            {
                if (!latestMarkers.TryGetValue(tail, out var currentMarker) || string.IsNullOrWhiteSpace(currentMarker))
                {
                    assessments.Add(new
                    {
                        ...
                        domainRiskSummary = "No parts row found for requested tail.",
                ...
                string rowJson = await TalkToPartsTable(tail);
                if (!string.IsNullOrWhiteSpace(rowJson) && rowJson != "{}")
                {
                    await SafeSetAsync(rowKey, rowJson);
                }

                string assessmentJson = await EvaluatePartsFromRowAsync(tail, rowJson);
            ...
            {
                systemStatus = "SYNCHRONIZATION_COMPLETE",
                updatedPartsAssessments = assessments
            };
        }
        ...
        private async Task<string> SafeGetAsync(string key)
        {
            ...
            {
                _logger.LogWarning(ex, "[PARTS SUBSYSTEM] Cache read failed for key {CacheKey}", key);
                return string.Empty;
            }
        }
        ...
        private async Task SafeSetAsync(string key, string value)
        {
            ...
            {
                _logger.LogWarning(ex, "[PARTS SUBSYSTEM] Cache write failed for key {CacheKey}", key);
            }
        }
        ...
        private async Task<Dictionary<string, string>> GetLatestMarkersAsync(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.parts_analytics` WHERE tail = @flightId GROUP BY tail";
                parameters = new[] { new BigQueryParameter("flightId", BigQueryDbType.String, targetFlightIds[0]) };
            }
            else
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.parts_analytics` WHERE tail IN UNNEST(@flightIds) GROUP BY tail";
        ...
        private async Task<List<string>> ScanTableForChanges(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.parts_analytics` GROUP BY tail";
            ...
            foreach (var row in results)
            {
                ...
                string cacheKey = $"parts_agent:flight:{flightId}:last_seen";
                string legacyCacheKey = $"parts_agent:flightId:{flightId}:last_seen";
                ...
                if (!cacheUnavailable)
                {
                    ...
                    catch (RpcException ex) when (ex.StatusCode == Grpc.Core.StatusCode.PermissionDenied)
                    {
                        ...
                        _logger.LogError("[PARTS SUBSYSTEM] Cache access denied. Continuing without cache. Verify GCP IAM for Datastore service account.");
                    }
                }
                ...
                if (!cacheUnavailable)
                {
                    ...
                    {
                        cacheUnavailable = true;
                        _logger.LogError("[PARTS SUBSYSTEM] Cache write denied. Continuing without cache. Verify GCP IAM for Datastore service account.");
                    }
                }
            }
        ...
        private async Task<string> EvaluatePartsFromRowAsync(string tail, string rowJson)
        {
                ...
                Contents =
                {
                    new Content
                    {
                        ...
                        Parts =
                        {
                            ...
                            {
                                Text =
                                    $"You are the Parts and Supply Chain Agent. Analyze the row for tail {tail}. " +
            ...
            GenerateContentResponse response = await Services.VertexRetry.InvokeAsync(
                () => client.GenerateContentAsync(request), _logger, "Parts");

            return response.Candidates[0].Content.Parts[0].Text;
        }

        private async Task<string> ExecutePartsAgentWithTools(string tail)
        {
            var client = await new PredictionServiceClientBuilder
            {
                Endpoint = $"{_location}-aiplatform.googleapis.com",
            ...
            var queryTool = new Tool
            {
                FunctionDeclarations =
                {
                    ...
                    {
                        Name = "QueryPartsTable",
                        Description = "Queries parts_analytics for tail-level supply chain and parts readiness metrics.",
            ...
            var request = new GenerateContentRequest
            {
                ...
                Contents =
                {
                    ...
                    {
                        Role = "USER",
                        Parts = { new Part { Text = $"You are the Parts & Supply Chain Agent. An update has occurred. Execute your table query tool for tail {tail}, review parts_status, part_required, part_available, part_in_transit, parts_issue and last_updated_at, and provide your safety risk verification assessment." } }
                    }
                },
                Tools = { queryTool },
                GenerationConfig = new GenerationConfig { ResponseMimeType = "application/json", ResponseSchema = responseSchema }
            ...
            {
                _logger.LogInformation($" -> [TOOL USE] Parts Agent requested execution of tool: {responsePart.FunctionCall.Name}");
                string requestedTail = responsePart.FunctionCall.Args.Fields["flightId"].StringValue;

                string tableDataJson = await TalkToPartsTable(requestedTail);
                ...
                var toolResponseRequest = new GenerateContentRequest
                {
                    ...
                    Contents =
                    {
                        ...
                        {
                            Role = "USER",
                            Parts = { new Part { FunctionResponse = new FunctionResponse { Name = "QueryPartsTable", Response = structuredResponseData } } }
                        }
                    },
                ...
                GenerateContentResponse finalAgentDecision = await Services.VertexRetry.InvokeAsync(
                    () => client.GenerateContentAsync(toolResponseRequest), _logger, "Parts");
        ...
        private async Task<string> TalkToPartsTable(string tail)
        {
            BigQueryClient client = await BigQueryClient.CreateAsync(_projectId, Services.GcpAuth.GetCredential());
            string query = @"SELECT tail, last_updated_at, parts_status, part_required, part_available, part_in_transit, parts_issue FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.parts_analytics` WHERE tail = @flightId ORDER BY last_updated_at DESC LIMIT 1";
            ...
            return JsonSerializer.Serialize(new
            {
                ...
                parts_status = row["parts_status"]?.ToString(),
                part_required = row["part_required"]?.ToString(),
                part_available = row["part_available"]?.ToString(),
