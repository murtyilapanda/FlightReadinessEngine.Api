...
namespace FlightReadinessEngine.Api.Agents
{
    public class GroundAgent
    {
        ...
        private const string CachePrefix = "ground_agent";
        ...
        public async Task<object> RunAsync(AgentSyncRequest? request)
        {
            ...
            if (!updatedTails.Any())
            {
                _logger.LogInformation("[GROUND SUBSYSTEM] No new row changes or cache mismatches detected.");
                return new { message = "Ground ops data is already synchronized. No analysis needed." };
            }

            _logger.LogInformation($"[GROUND SUBSYSTEM] Processing updates for {updatedTails.Count} aircraft tail(s): {string.Join(", ", updatedTails)}");
        ...
        private async Task<object> RunTargetedCacheFirstAsync(List<string> targetTails)
        {
            ...
            foreach (var tail in targetTails)
            {
                if (!latestMarkers.TryGetValue(tail, out var currentMarker) || string.IsNullOrWhiteSpace(currentMarker))
                {
                    assessments.Add(new
                    {
                        ...
                        domainRiskSummary = "No ground ops row found for requested tail.",
                ...
                string rowJson = await TalkToGroundTable(tail);
                if (!string.IsNullOrWhiteSpace(rowJson) && rowJson != "{}")
                {
                    await SafeSetAsync(rowKey, rowJson);
                }

                string assessmentJson = await EvaluateGroundFromRowAsync(tail, rowJson);
            ...
            {
                systemStatus = "SYNCHRONIZATION_COMPLETE",
                updatedGroundAssessments = assessments
            };
        }
        ...
        private async Task<string> SafeGetAsync(string key)
        {
            ...
            {
                _logger.LogWarning(ex, "[GROUND SUBSYSTEM] Cache read failed for key {CacheKey}", key);
                return string.Empty;
            }
        }
        ...
        private async Task SafeSetAsync(string key, string value)
        {
            ...
            {
                _logger.LogWarning(ex, "[GROUND SUBSYSTEM] Cache write failed for key {CacheKey}", key);
            }
        }
        ...
        private async Task<Dictionary<string, string>> GetLatestMarkersAsync(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.ground_analytics` WHERE tail = @flightId GROUP BY tail";
                parameters = new[] { new BigQueryParameter("flightId", BigQueryDbType.String, targetFlightIds[0]) };
            }
            else
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.ground_analytics` WHERE tail IN UNNEST(@flightIds) GROUP BY tail";
        ...
        private async Task<List<string>> ScanTableForChanges(List<string> targetFlightIds)
        {
            ...
            {
                query = @"SELECT tail, CAST(UNIX_MICROS(MAX(last_updated_at)) AS STRING) as latest_marker FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.ground_analytics` GROUP BY tail";
        ...
        private async Task<string> EvaluateGroundFromRowAsync(string tail, string rowJson)
        {
                ...
                Contents =
                {
                    new Content
                    {
                        ...
                        Parts =
                        {
                            ...
                            {
                                Text =
                                    $"You are the Ground Operations Agent. Analyze the row for tail {tail}. " +
            ...
            GenerateContentResponse response = await Services.VertexRetry.InvokeAsync(
                () => client.GenerateContentAsync(request), _logger, "Ground");
        ...
        private async Task<string> ExecuteGroundAgentWithTools(string tail)
        {
            ...
            if (responsePart.FunctionCall != null)
            {
                ...
                GenerateContentResponse finalAgentDecision = await Services.VertexRetry.InvokeAsync(
                    () => client.GenerateContentAsync(toolResponseRequest), _logger, "Ground");
        ...
        {
            BigQueryClient client = await BigQueryClient.CreateAsync(_projectId, Services.GcpAuth.GetCredential());
            string query = @"SELECT tail, last_updated_at, ground_ops_status, cargo_loading_complete, weight_balance_cleared, ground_ops_issue FROM `qwiklabs-gcp-04-509f741dc909.aviation_ops_analytics.ground_analytics` WHERE tail = @flightId ORDER BY last_updated_at DESC LIMIT 1";
